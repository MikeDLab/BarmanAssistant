...
public class FindAverageBarmanRatingSet extends AbstractRatingSpecification implements RatingSpecification {
	private final static String FIND_BARMAN_RATING_SET_BY_USER_ID = "SELECT barman_rating, barman_id, user_id FROM (SELECT user_id as barman_id FROM User) User  INNER JOIN BarmanRating USING(barman_id) WHERE barman_id = ?";
	private int userId;
